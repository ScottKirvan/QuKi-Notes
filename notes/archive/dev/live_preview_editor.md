...
- Moving the cursor into a rendered piece with the arrow keys reveals it — you then navigate through its actual source characters. Moving past the far end collapses it again. You never arrow through invisible characters you can't see.
- Tapping into a rendered piece reveals it and places the cursor at the character you actually tapped on — the same behavior as tapping into bold text in Obsidian. This works because we own the rendering and know exactly where each character sits on screen, so we can map a tap position back to its source character precisely. The one exception is tapping on a rendered image (pixels inside an image have no corresponding source character), where we fall back to the nearest boundary instead.
...
4. **Arrow-key movement into a rendered piece reveals it.** If a link is showing as just its label, pressing the right-arrow key to enter it from the left reveals the full source — `[label](url)` — and you then navigate character-by-character through all of it. Moving past the closing `)` collapses it back. You never arrow through invisible characters; you either reveal the element and navigate its real text, or you're outside it entirely.
5. **Tapping into a rendered piece reveals it and places the cursor at the character you tapped on.** Because we own the paint pass, we know where every rendered character sits on screen and we know how rendered positions map back to source positions — so a tap can resolve to a precise source offset, not just a boundary guess. (Tapping on a rendered image is the exception: a pixel inside an image has no source character, so we fall back to the nearest boundary in that case.)
