...
1. **Correct word/entity selection + double-tap.** Implemented and merged — PR #320, 2026-08-01. Root cause of the long-press partial-word bug: the old word-boundary scan ran over raw *source* text (still containing hidden markdown delimiters), not *rendered* text — `*` correctly fails a `\w` test, so the scan stopped dead at the first hidden delimiter. Fixed by scanning rendered text and mapping back through `RenderModel`'s existing offset maps. Long-press and double-tap now share one implementation (`_selectEntityAt`), verified by a test that asserts identical results from both gestures for the same input. **Known, accepted limitation**: the numeric-entity pattern (digits + `. - / ( )`) has no space in its character class, so a realistically-formatted phone number like `(555) 123-4567` selects as two fragments, not one. Confirmed acceptable by the project owner — Stage 2's handles give a manual way to bridge that gap, and it's not worth blocking on. **Resolved via device testing**: adding a double-tap recognizer to the same gesture arena as the existing tap recognizer measurably delays `onTapDown` in test timing (up to ~300ms) — confirmed real, not hypothetical, during review. Device-tested by the project owner after merge: **tap-to-cursor placement feels fine, no perceptible lag.** No further action needed on this — closed.
2. Draggable selection handles + crossing — implemented and merged, PR #323, 2026-08-03. Deliberately scoped down from full §2: the single collapsed-cursor drag handle is held back to a later pass, keeping this stage focused on adjusting an existing selection. Shipped after two real-device-driven fix rounds on the same branch (see `notes/dev/decisions.md` → ADR-36 for the full account): a scroll-position-staleness bug (real, but not the actual reported cause) and the real root cause — handles are painted one line-height below the caret they represent, but touch resolution wasn't correcting for that offset, so grabbing a handle exactly where it was drawn resolved onto the wrong line. Device-tested and accepted, with a note there may be minor tweaks wanted later (not yet itemized).
3. Auto-scroll while dragging a handle near a viewport edge — implemented and merged, PR #325, 2026-08-03. Device-tested and accepted on the first attempt, no fix rounds needed. Built on Stage 2 Round 1's post-frame-deferred scroll-notification machinery to keep the dragged boundary resolving correctly against a stationary finger while content scrolls continuously underneath it.
4. Magnifier + haptic polish — implemented and merged, PR #327, 2026-08-03. **Not dropped**: the magnifier's open feasibility question resolved favorably (Flutter's `RawMagnifier`/`MagnifierController` really are usable without `RenderEditable` — confirmed by reading the SDK source, not assumed), a real working implementation was built, and it was **device-tested and confirmed by the project owner**. This closes ADR-36 — all four stages are now shipped.

## Still open (not resolved by "adopt Android's behavior as requirements")

- **Reading mode**: reading mode currently hides the cursor and disables the edit gesture set entirely, so none of ADR-36's selection work functions there. Filed as **#328**. Whether selection (for copying) should get the full toolbar or a narrower Copy/Select-All-only subset (matching Android's read-only `TextView` convention) is still unresolved — the issue records the question, doesn't answer it.
- **Single collapsed-cursor drag handle**: part of the original confirmed §2 spec, deliberately held out of Stage 2's scope to keep it focused on the two-handle selection case, never picked back up in Stage 3 or 4. Filed as **#329**.
- **Desktop (Windows/Linux) equivalents**: this document is Android-specific by design. Double-click-for-word and triple-click-for-paragraph are common desktop conventions but — per §7 — aren't a single unambiguous platform standard the way Android's gestures are. Needs its own decision, not a straight port of the Android spec. Not yet filed as an issue — still a design question, not a confirmed gap.

(§1a's punctuated-numeric-string character class was resolved during Stage 1 — digits-plus-punctuation only, no letters — see `decisions.md` → ADR-36 "Rejected/deferred" for the reasoning. No longer listed here as open.)
