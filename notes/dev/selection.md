...
2. Draggable selection handles + crossing — briefed (`Agents/quiki-dev/CLAUDE.md`), in progress. Deliberately scoped down from full §2: the single collapsed-cursor drag handle is held back to a later pass, keeping this stage focused on adjusting an existing selection.
