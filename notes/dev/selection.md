...
4. Magnifier + haptic polish — explicitly provisional: try it for a round or two; if it isn't landing, it can be dropped, *unless* stages 2/3 turn out to genuinely need it for usable cursor/handle placement rather than just polish. **"Isn't landing" means a real, working implementation gets built and actually tried on a real device and found not to work well — not a code-level judgment that it looks hard or awkward to build. A build-it-first-then-decide requirement, not an easy out.** (Corrected by the project owner 2026-08-04 after an initial brief was too permissive about this.)
