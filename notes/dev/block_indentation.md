...
## Stage 4 — Interactive Indent/Dedent: toolbar buttons + Tab/Shift+Tab (#77)

Stages 1-3 built the rendering and parsing foundation (real per-line `indentLevel`, wrap-correct layout) and made the Tab *key* reach the text buffer at all (previously silently swallowed by Flutter's default focus-traversal, now inserts a literal tab character at the cursor). Neither shipped the thing #77 actually asked for: an interactive way to indent/dedent a line. This section specifies that behavior in full — it is genuinely new interactive-editing behavior, not a rendering concern, so (per this project's own brief-writing rule) it needs to be nailed down completely here rather than left for the implementer to infer.

**Two entry points, one action**: a new **Indent** / **Dedent** button pair in `FormattingToolbar` (alongside the existing List/Ordered-list/Checklist buttons — this is the primary, mobile-reliable entry point, since a physical Tab key isn't a given on touch devices), and the **Tab**/**Shift+Tab** keys, which trigger the exact same underlying action. There is no behavioral difference between pressing the button and pressing the key.

**The core operation is a line-start whitespace prepend/strip, not a cursor-position insert** — a deliberate change from how the Tab key currently works (which inserts at the cursor). Indent/Dedent always act on whichever line(s) the cursor or selection touches, at the *start* of each line, regardless of where the cursor happens to sit within that line's content. This matches how Indent/Outdent works in Word, Google Docs, and VS Code, and is the expected mental model for the classic "type `-`, press Enter, press Indent" outlining workflow.

**What "line start" means differs by what's already on the line — this is not one uniform rule, and naively applying one breaks things**:

- **List items** (`ul`/`ol`/`checkboxUnchecked`/`checkboxChecked`, at any existing depth): the whitespace goes *before* the existing marker — this is exactly what already controls nesting depth (`_listIndent`), so Indent/Dedent here means real depth change, identical in effect to what was originally scoped for this stage. Dedent clamped at level 0 — never negative, never converts the line to something else. The newly auto-continued (and still empty) line from Enter is itself just "a list-item line," so the classic type-Enter-Indent workflow is covered with no special-casing.
- **Plain paragraph lines** (no recognized block marker at all): the whitespace goes at the absolute start of the line — a literal tab character inserted/removed there. **Known, deliberate divergence from real GFM/CommonMark**, logged as #305: strict CommonMark would either turn a 4+-space/tab-indented line that starts a new block into an *indented code block*, or (if it's a lazy continuation of an existing paragraph) silently discard the leading whitespace entirely — this app does neither; it does a plain literal-tab-insert, which visually nudges only the *first* visual row of a wrapped paragraph, the same class of wrap-incorrectness bug ADR-34's earlier stages exist to fix for blockquote/list. Shipping this now, deliberately, per the project owner's explicit call to log and defer rather than block on it — do not attempt to fix the wrap-correctness gap as part of this stage, that's #305's problem once code-block support (#244) is scoped.
- **Headings and blockquotes**: excluded. Prepending whitespace *before* their marker (`#`, `>`) would break recognition entirely (`line.startsWith('#')`/`startsWith('>')` would fail, silently falling through to plain-paragraph handling and rendering the literal marker character) — a real regression, not a style choice. Indent/Dedent on these lines instead falls back to the pre-existing Tab behavior, unchanged: insert a literal tab character at the cursor position (not line start) for Indent; Dedent (Shift+Tab or the Dedent button) is a no-op on these lines, since there's nothing to remove.
- **Active multi-line selection**: apply the appropriate per-line rule above (list-item depth change, paragraph line-start insert, or heading/blockquote exclusion) independently to every line the selection touches, even partially — a selection spanning list items at different existing depths keeps their relative nesting to each other after the shift. If the selection touches **zero** lines that qualify for a real line-start operation (i.e., it's entirely inside a heading/blockquote or otherwise has nothing to do), fall back to the existing selection-replace-with-tab-character behavior, unchanged.
- **Ordered-list numbering and marker rendering after an Indent/Dedent on a list item**: no new logic needed — once a line's leading whitespace changes, the existing parser (`_listIndent`, depth-scoped ol numbering, gutter-decoration marker painting) already recomputes everything correctly on the next parse pass, exactly as if the user had typed or deleted that whitespace by hand.

**Explicitly out of scope for this feature:**

- **"Indent with children"** — recursively indenting a list item's nested sub-items along with it (Workflowy/Notion-style outliner semantics) is *not* part of this feature. This codebase's model is a flat per-line `indentLevel`, not a real parent-child tree, so Indent/Dedent only ever affects the line(s) actually touched by the cursor or selection — nothing implicitly follows along. If a user wants to move a sub-tree, they select it explicitly (covered by the multi-line-selection rule above).
- **Blockquote nesting depth** (`>`, `>>`) — untouched by this feature. Blockquote depth stays purely keyboard-typed (literal `>` characters), unrelated.
- **Wrap-correct paragraph indentation** (making paragraphs participate in the real `indentLevel`/run-based layout mechanism the way list/blockquote do) — tracked as #305, not this stage.
- Any visual "outdent guide" or other UI affordance beyond the button/keystroke behavior itself.
...
| 4 — **briefed**, branch TBD | Interactive Indent/Dedent: new toolbar buttons + Tab/Shift+Tab, both triggering the same line-start whitespace prepend/strip action (full behavior spec above) — real depth change for list items, literal-tab-insert for plain paragraphs (logged GFM divergence, #305), headings/blockquotes excluded and fall back to the pre-existing cursor-insert Tab behavior | Pressing Indent (button or Tab) on a list item nests it a level; Dedent (button or Shift+Tab) un-nests it; the classic type-Enter-Indent outlining workflow works end to end; the same buttons work on mobile with no keyboard | #77 |
