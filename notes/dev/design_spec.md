...
- **Tap-to-source-character**: tapping inside a collapsed element's rendered bounds reveals the element and places the cursor at the source character corresponding to the tap position. We own the paint pass and therefore know both the rendered character positions and the rendered→source offset mapping; a tap resolves to a rendered character, which maps to a source offset. Fallback for non-text rendered content (embedded images): tap resolves to whichever source boundary is nearer the tap's x-coordinate, since there is no character mapping for pixels inside an image.
...
| 3 | Boundary-reveal cursor movement + tap-to-source-character hit-testing — arrow key at boundary reveals source; tap inside rendered element reveals and places cursor at the tapped source character; exit far boundary collapses | Cursor navigation feels correct, not just visually correct |
