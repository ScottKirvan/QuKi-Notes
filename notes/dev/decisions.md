...
**Status**: implemented (PR #314, `feat/html-paste-conversion`); clipboard-reading dependency swapped mid-review after the first implementation's CI surfaced a real build failure — see "Superseded sub-decision" below. Not yet merged as of this entry.
...
Replaced with **`quill_native_bridge`** (standalone package from the `flutter_quill` ecosystem, not a dependency on the Quill editor itself) — `getClipboardHtml()`. Verified by reading its actual source, not just claims: Android is a genuine Kotlin platform channel (plain `com.android.library` Gradle plugin, no cargokit); Windows calls the real win32 clipboard API via FFI (`OpenClipboard`/`GetClipboardData`, no native compilation step); Linux shells out to a bundled `xclip` binary — functionally real, but with one acknowledged upstream gap: no native Wayland support yet (their own tracked issue, still open). Low-severity here specifically because this ADR's own fallback design (any HTML-read failure degrades to the existing plain-text paste path) already contains that failure mode — a Wayland-only Linux setup just doesn't get the paste enhancement, it doesn't crash or fail to build. Actively maintained (pushed within the last two weeks at time of research), 160/160 pub points, MIT. `super_clipboard` and `html2md` were the pair locked in this ADR's first pass; only the clipboard-reading half changed — `html2md` is unaffected and still in use.
