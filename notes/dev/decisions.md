...
- **Tap-to-source-character hit-testing**: tapping inside a collapsed element's rendered bounds reveals the element and places the cursor at the source character corresponding to the tap position. This is achievable because we own the paint pass and know both the rendered character positions and the rendered→source offset mapping for each element type; a tap resolves to a rendered character, which maps directly to a source offset. Fallback for non-text rendered content (e.g., an embedded image widget): tap resolves to whichever source boundary (start or end) is nearer the tap's x-coordinate, since there is no character-level mapping for pixels inside a rendered image.
...
- **Why**: Lucide's stroke style is cleaner than Material's filled defaults for this app's aesthetic. `Icons.*` remain available for anything not yet migrated.
- **Rejected**: `material_symbols_icons` (Google's newer set, heavier package); Lucide-via-SVG assets (requires custom rendering, no `Icon` widget compatibility); staying with `Icons.*` (rejected in favour of Lucide).
