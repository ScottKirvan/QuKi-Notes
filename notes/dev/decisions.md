...
**Status**: Stage 1 (multi-run rendering foundation + nested blockquotes) complete — PR #292, merged 2026-07-23, device-tested and confirmed by the project owner before merge, closed #242 and #237. Stage 2+3 (list-item indent detection, wired directly into Stage 1's rendering foundation — combined into one brief, see `block_indentation.md`) briefed 2026-07-23, branch `feat/block-indentation-stage2`. Stage 4 (Tab/Shift+Tab, #77) not yet briefed. Full spec: `notes/dev/block_indentation.md` — read that first; this entry is the index pointer.
