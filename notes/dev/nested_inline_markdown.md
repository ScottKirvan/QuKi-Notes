...
**Scope decision for Stage 3 (2026-07-21)**: full CommonMark HTML-block detection is seven distinct block-start conditions (`<script>`/`<pre>`/`<style>`/`<textarea>`, comments, processing instructions, declarations, CDATA, a fixed list of ~40 block-level tag names, or any tag alone on its line), several of which can span multiple lines until a matching close appears on a *later* line. QuKi-Notes' parser has no multi-line state-tracking today except the narrow ordered-list-block counter — building general multi-line span tracking is a bigger architectural lift than this stage's actual goal (preventing a misparse, not rendering HTML). Scoping Stage 3 to single-line detection only:
- A **line consisting entirely of one or more HTML tags/comments** (`<div class="x">`, `<!-- note -->`, `</span>`) is excluded from inline scanning for that line — shown exactly as typed, no styling, same as how it already looks today (this needs no render-layer change — it only needs `MdParser` to *not* call the inline scanner on that line).
- An **inline HTML tag appearing mid-line** (`text <span style="color:red"> more text`) is skipped over by the inline scanner — the tag span is excluded from emphasis/link matching, same skip mechanism already used for inline code spans, without needing a dedicated visual treatment either.
- A permissive single-line heuristic is enough — `<` or `</` followed by an ASCII letter or `!`, ending at the next unescaped `>` — not real HTML grammar validation.
- **Not attempted**: HTML blocks that span multiple lines until a later closing line (`<script>`...`</script>` across several lines, multi-line comments). If this turns out to matter in practice, it's its own follow-up, not silently bundled in here.
...
| 3 | Single-line HTML detection (block + inline) — markdown scanning suppressed on a line/span that's entirely an HTML tag/comment, passed through raw. Multi-line HTML blocks explicitly not attempted (see "HTML — detect, don't parse" above) | Closes the `*gray*`-inside-an-attribute misparse risk for the common single-line case | **Complete** (PR #281, merged 2026-07-22) |
| 4 | Blockquote content flows through the same engine, same pattern as Stage 2 (mirrors list-item integration), plus three rounds of rendering-bug fixes (bare `>` marker, empty-blockquote stripe, wrapped/multi-line stripe continuity, content indentation, stripe vertical alignment) | `> **important**` renders bold instead of literal asterisks; corrects the original spec's wrong "blockquotes are out of scope" exclusion; rendering matches GitHub except one known limitation (below) | **Complete** (PR #283, merged 2026-07-22) |
...
**Known limitation shipped with Stage 4**: blockquote content indentation only affects the first rendered line of a wrapped source line — a long blockquote line that word-wraps has its first visual row indented but subsequent wrapped rows snap back to the unindented margin. Root cause: the whole document renders through one continuous `TextPainter` with no per-block width-constrained layout region; leading blank characters (the mechanism used for the indent) only affect where a logical line's rendered text begins, and Flutter's automatic word-wrap doesn't reapply that trick to wrapped continuation rows. This is the identical constraint blocking real list-nesting indentation (#241) — documented on that issue (2026-07-22 comment) rather than re-litigated here. Not planned to be fixed as a standalone patch; the real fix is scoped together with #241.
