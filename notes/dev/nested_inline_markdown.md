...
- Backslash escapes (`\*not italic\*`) — CommonMark treats this as core inline syntax, but it's not currently supported at all and wasn't part of the original ask. Flagging it here as an open question rather than silently including or excluding it: worth a decision (in scope now, or its own follow-up) before the dev brief is written.
...
Adapt relevant cases from CommonMark's official spec test suite (publicly published, versioned, JSON input/expected-output pairs) for the supported subset — emphasis/strong resolution, intraword rules, code-span precedence, nested nesting — as permanent regression tests, in addition to QuKi-Notes-specific cases (list-item content, heading content, the whole-chain reveal rule). "Does this match GitHub" should be checkable against a cited spec example, not eyeballed.
