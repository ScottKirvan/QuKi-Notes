...
| | 3.48 Nested inline markdown Stage 4 — blockquote content + rendering fixes (ADR-33) | Complete (PR #283, merged 2026-07-22) — known limitation: wrapped-line indent, see `nested_inline_markdown.md` |
...
## Implementation Notes (current as of v0.18.2 + PRs #257–#260, #270, #276, #279, #281, #283 merged; #262 release-please pending)
...
**Blockquote rendering (Stage 4)**: `MdElKind.blockquote`'s marker length is now variable (`_srcMarkerLen`, shared with `ol`) — 1 char for a bare `>`, 2 for `> `, matching CommonMark's actual rule (previously required the space always). `BlockquoteSlot` recording moved from an in-loop check (which had an off-by-one causing empty blockquotes to paint no stripe) to a post-pass over the completed offset map. `groupBlockquoteRuns()` merges stripe slots for consecutive `>` lines into one continuous border, matching GFM. The stripe's vertical extent uses `TextPainter.getBoxesForSelection(BoxHeightStyle.tight)` (glyph ink bounds) rather than `getOffsetForCaret` (line-box bounds, which sit above the ink when `height > 1.0` — this editor uses `height: 1.4`). Blockquote content gets a `collapsedMarker` of 4 blank characters to reserve horizontal indent, reusing the same mechanism as checkbox/list markers. **Known limitation**: this indent only applies to a line's first visual row — wrapped continuation rows are not indented, because indentation via leading characters can't be reapplied after Flutter's automatic word-wrap, and the whole document renders through one `TextPainter` with no per-block width-constrained region. Same root constraint as list-nesting (#241); documented there, not fixed as a standalone patch.
