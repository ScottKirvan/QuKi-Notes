...
**PRs open**: none as of this sync (2026-08-10) — check `gh pr list` fresh, as always, rather than trusting this being kept current in real time.

**Merged since prior sync — checkbox tap + text selection reading-mode safety, four device-tested rounds (#335, #266, #336, #352, #354)**:
- **PR #350** — one bundled branch, kept as a single PR throughout rather than split, per the project owner's explicit direction ("if this touches on other bugs, we should batch them in") — even after two separate stray PRs (#353) got opened by mistake mid-effort and had to be fast-forward-merged back in. Four rounds:
  - **Round 1** (#335, #266, #336): checkbox tap no longer reveals raw source / scrolls to top / opens the keyboard; text selection via long-press/double-tap in reading mode no longer opens the keyboard. Root cause for the first two: `MarkdownEditorController.setValue()`'s hardcoded selection-reset-to-`0`, applied to an in-place checkbox edit it was never meant for — new `setValuePreservingSelection()`. Root cause for #336: `_onTapDown`'s eager focus request also fires ahead of a long-press/double-tap's own gesture resolution; reversed post-hoc via a focus-state-at-raw-pointer-down capture.
  - **Round 2, three attempts on the checkbox hit-test zone** (#352): attempt 1 (bounded to the 24px marker gutter) shipped, device-tested, and found to add only ~2px of real margin — its own test proved "doesn't overlap a neighbor," never "is actually big enough," so it passed while not fixing anything meaningful. Caught by me only after device-test feedback, not before — a real review gap I own plainly rather than gloss over. Attempt 2 (anchored at the row's text-origin left edge) fixed the nested-vs-non-nested gap but was *also* device-tested and found still insufficient — it never accounted for the editor's own 12px content padding sitting further left, exactly where a real "farthest left I can tap" lands. Attempt 3/round 4 (anchored at the actual widget edge, padding included) is the one that worked — traced by me before the brief went out this time, so the diagnosis was confirmed going in rather than hypothesized.
  - **Round 3** (#354): found mid-investigation, not originally in scope — nested checkboxes didn't visibly toggle even with a perfectly-targeted tap, because `_onCheckboxToggle`'s marker-read assumed the marker started exactly at the tap's resolved offset (true only for non-nested items; `MdElement.start` is the *line's* absolute start, before any leading indentation whitespace). Fixed by skipping leading whitespace before reading the marker. A new test's flaky full-suite-only failure (never reproduced in isolation) was root-caused to two genuine timing issues — real disk-write latency under contention, and a Windows-specific transient exception during the storage layer's write-temp-then-rename — and fixed with a real poll-and-retry loop, independently verified by me across three consecutive clean full-suite runs, not just trusted from the report.
  - Every round's diff was read in full and every test suite re-run independently by me before folding it into the PR, not trusted from any implementation report alone — including tracing `_RunLayout.x`'s actual derivation in `performLayout` myself for round 2's second attempt, and `md_parser.dart`'s indented-checkbox branch myself for round 3, before accepting either diagnosis.
  - Surfaced, filed, but correctly left unfixed (out of scope for this effort): **#349** (link-tap has the identical eager-focus-before-hit-test pattern checkboxes had) and a partial-progress note added to **#328** (text selection in reading mode — selecting no longer opens the keyboard, but whether handles/toolbar render, and toolbar scope, are still open).
  - Five closed issues; GitHub's auto-close only caught 3 of them (a bold-markdown-around-the-issue-number quirk broke its keyword parser for #266 and #336) — caught and closed manually rather than left dangling.

**Process note worth remembering**: launched two of this effort's implementation rounds with `isolation: "worktree"` after the first round (launched without it) caused a real collision — the agent, sharing my own working directory, ran its own `git checkout` mid-task and stashed (not lost, but disruptive) my own concurrent uncommitted doc edits. Always use worktree isolation for any background agent that touches the filesystem while I'm also actively editing.

**Merged since prior sync — "Toss" → "Transport" rename + non-functional cold-launch auto-focus removed**:
- **PR #347** (code, `refactor/rename-toss-to-transport`) + **PR #348** (docs, `docs/rename-toss-to-transport`) — every "Toss"/"QuKi-Toss" identifier renamed to "Transport" across `lib/`, `test/`, and all project docs, per the project owner's direction that it "has to look like it's never existed." Zero behavior change; deliberately did not introduce "Send" as the replacement anywhere in the rename (that word only stays where it was already correct — the real UI button, the tagline — a distinction the project owner corrected me on mid-effort after I asked whether the interface method should be `send()` or `transport()`).
- **PR #346** — the cold-launch auto-focus code (`_EditorScreenState.initState()`'s `postFrameCallback` `requestFocus()`, shipped as "Complete" across Phase 3.11/3.20/3.20a/3.38) never actually worked on a real device per the project owner ("has never worked correctly") — removed outright rather than left in place implying it functions. Four widget tests broke on removal (silently relying on the same dead code for focus/settle timing); root-caused to a real, separate, already-documented gesture-arena delay and fixed using the exact pattern already established elsewhere in the suite, not papered over with arbitrary extra pumps.
- Also corrected two real documentation inaccuracies found during this pass, independent of any code change: root `CLAUDE.md`'s storage-layer description claimed a `<storage-root>/qukis/{uuid}.md` nested layout that doesn't match production (flat, no `qukis/` subfolder — confirmed by tracing `QuKiStorage._mdFile()`), and image rendering (Phase 3.30) was documented as shipped/working when it has never actually rendered a real image (traced to a storage-layout/ADR-4 path-convention mismatch, filed as #344).

**Merged since prior sync — QuKis list Help button + Ko-fi link + tighter density**:
- **PR #331** (`feat/stream-help-kofi-density`, merged 2026-08-05, shipped as `fix:` per the project owner's explicit call at commit time) — small, self-contained UI change the project owner made directly on `main` rather than routing through an Implementation session brief: Help button added to the QuKis (Stream) screen app bar (mirrors the Editor screen's existing entry point into the same help dialog), a Ko-fi "Buy me a coffee" link row added to the help dialog itself, and `ListTile` vertical density tightened on the QuKis list. Reviewed directly (no sub-agent involved) — confirmed Lucide-only iconography, no new color tokens, no vault-chrome creep, matched the existing Discord/GitHub link-row pattern for the new Ko-fi row. `just lint` initially failed on unrelated cause: `packages/markdown_live_editor`'s dependencies (`html`, `html2md`, `quill_native_bridge`) weren't resolved in this checkout; fixed with `flutter pub get` in that package directory, unrelated to the two changed files. Root test suite 115/115 passing after. A stray 7-file CRLF/LF-only diff in the generated plugin-registrant files (`linux/`, `macos/`, `windows/`) from local `flutter` commands was discarded before committing — zero content diff, same pattern seen during Stage 4 verification.
