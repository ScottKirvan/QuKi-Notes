...
**PRs open:**
- #277 (`release-please--branches--main--components--quki_notes`) — auto-generated release-please bundling PR. Likely stale/superseded by a newer release-please PR by now — check `gh pr list` fresh rather than trusting this line, this file's "Current version" section above has not been kept in sync with the actual release cadence.
- **#310** (`docs/pumpwidget-gotcha-note`) — CI green. Documents the `pumpWidget()`-reuse widget-test gotcha (see PR #309's entry below) in `notes/dev/testing.md`. Docs-only.
- **#311** (`test/fix-pumpwidget-reuse`) — CI green. Fixes the one confirmed vacuous test found via #310's investigation (`block_indentation_test.dart`'s marker-gutter-leak comparison test) by adding `key: UniqueKey()` to the file's shared `buildEditor()` helper. Re-verified the underlying fix is still correct once the test genuinely remounts both documents.
