...
**ADR-34 (real block indentation) status**: **Stage 1 and Stage 2+3 both complete and merged** (PR #292, PR #294) — closed #242, #237, and #241 (the ADR's original motivating issue). See `notes/dev/block_indentation.md` for the full spec and `notes/dev/decisions.md` → ADR-34 for the up-to-date status summary. **Stage 4 (Tab/Shift+Tab interactive indent/dedent, #77) not yet briefed** — #77 stays open; PR #294 only fixed Tab-as-a-keystroke (it now types a literal tab character), not "press Tab anywhere in a list item to indent that item" behavior.
