...
**This stage is explicitly provisional — read this before you start building.** Per `selection.md`'s staging plan: try the magnifier for a round; if it isn't landing, it can be dropped. Stages 2 and 3 both shipped and were confirmed usable on a real device *without* a magnifier, so the escape clause that would force it ("unless stages 2/3 turn out to need it for usable handle placement") does not apply — this is now genuinely optional polish, not a blocking requirement. Magnifier and haptics are two independent, separable pieces — evaluate and report on each on its own merits. If the magnifier turns out to be expensive or awkward to build well, ship haptics alone and report back on the magnifier rather than forcing it in or silently dropping it without saying so.
